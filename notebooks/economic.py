...
@app.cell
def _(parse_economic_calendar, extract_calendar_table):
    import curl_cffi as curl
    
    url = "https://www.forexfactory.com/calendar"
    response = curl.get(url, impersonate="chrome")
    if response.status_code != 200:
        print("Failed to fetch economics calendar: HTTP %d", response.status_code)
        return []
    table = extract_calendar_table(response.text)
    parse_economic_calendar(table)
    return
